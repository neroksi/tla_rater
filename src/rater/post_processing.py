...
# def get_preds_seg_experimental(
#     uuids: list,
#     res: pd.DataFrame,
#     res_seg: pd.DataFrame,
#     span_getter: Callable = None,
...
# ) -> dict:
#     """
#     Process segmentation predictions to extract and refine spans.

#     Parameters
#     ----------
#     uuids : list
#         Series of unique identifiers.
#     res : pd.DataFrame
#         DataFrame containing initial results.
#     res_seg : pd.DataFrame
#         DataFrame containing segmentation targets.
#     span_getter : Callable, optional
#         Function to get spans from segmentation, by default get_spans_from_seg_v3.
#     target_getter : Callable, optional
#         Function to get targets from spans, by default get_targets_from_spans_v2.
#     span_getter_kwargs : dict, optional
#         Additional keyword arguments for span_getter function.
#     target_getter_kwargs : dict, optional
#         Additional keyword arguments for target_getter function.

#     Returns
#     -------
#     dict
#         Dictionary containing all spans with associated IDs.
#     """
#     span_getter = get_spans_from_seg_v3 if span_getter is None else span_getter
#     target_getter = (
#         get_targets_from_spans_v2 if target_getter is None else target_getter
...
#     # Convert column names for standardization
#     res = res.copy()
#     res.columns = [to_1_7(col) for col in res.columns]
#     res = res.groupby(level=0, axis=1).sum()
...
#         # Get spans using the specified span_getter function
#         spans = span_getter(
#             seg_target, pred_target=res_v0.loc[index].values, **span_getter_kwargs
#         )

#         # Get refined targets using the specified target_getter function
#         spans = target_getter(spans, preds=df_uuid.values, **target_getter_kwargs)

#         id_ = uuids[df_uuid.index[0][0]]
#         for i in range(
#             len(spans) - 1, -1, -1
#         ):  # Iterate in reverse order to avoid popping issues
#             span = spans[i]
#             if span["class_id"] <= 0:
#                 spans.pop(i)
#             else:
