...
class BaseModel(nn.Module):
    def __init__(
        ...
        pretrained=False,
        use_position_embeddings=True,
        # resize_before_load_checkpoint=False,
    ):
        super().__init__()
...
class NERSegmentationLoss(nn.Module):
    ...
    def forward(self, out_ner, out_seg, out_eff, target, eff_target, num_calls=None):
        ...
        eff_target = torch.stack([eff_target] * out_eff.size(1), dim=1)
        ...
        l_eff = eff_loss(out_eff[eff_bools], eff_target[eff_bools])

        l = (
            cfg.ALPHA_NER * l_ner + cfg.ALPHA_SEG * l_seg + cfg.ALPHA_EFF * l_eff
        )  # noqa: E741
