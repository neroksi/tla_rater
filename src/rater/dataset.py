...
def read_train_df(train_csv_path=None):
    ...
    df["predictionstring"] = (
        ...
        .fillna("")
        .astype(str)
        .apply(lambda x: np.fromstring(x, sep=" ", dtype=np.int16))
    )
...
def add_training_fold(df, nfolds=5, group_col="essay_id", seed=None):
...
def prediction_string_is_contiguous(df):
    ...
    return (
        df["predictionstring"].apply(
            lambda x: np.sum(x[1:] - x[:-1] - 1) if len(x) > 1 else 0
        )
        > 0
    ).all()
...
def gen_data_from_id(uuid, tokenizer, append_bar, df=None, root=None, text: str = None):
    """
    ...
    # if len(append_bar):
    #     tokens = [f"|{t}" if t in append_bar else t for t in tokens]
...
class Dataset(TorchDataset):
    ...
    def add_special_tokens(self, input_ids, target, eff_target):
    ...
    def truncate(self, word_perc_pos, input_ids, target, eff_target):
...
class TestDataset(Dataset):
    ...
    def add_special_tokens(self, word_ids, input_ids, target=None, eff_target=None):
    ...
    def truncate(
        self, word_ids, word_perc_pos, input_ids, target=None, eff_target=None
    ):
